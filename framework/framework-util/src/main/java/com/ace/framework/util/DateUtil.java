...
public class DateUtil {
     ...
     * @return 两个时间相差的天数
     */
    public static int calculateDayCnt(Date before, Date after) {
        if (before == null) {
            throw new IllegalArgumentException("The before must not be null!");
        }
        ...
        int dayMillSeconds = 1000 * 60 * 60 * 24;

        return (int)Math.abs(time / dayMillSeconds);
    }

    public static int calculateDayCntWithoutAbs(Date before, Date after) {
        if (before == null) {
            throw new IllegalArgumentException("The before must not be null!");
        }
        ...
        return (int)time / dayMillSeconds;
    }

    public static int calculateMinCnt(Date before, Date after) {
        if (before == null) {
            throw new IllegalArgumentException("The before must not be null!");
        }
        ...
        long time = after.getTime() - before.getTime();
        int minMillSeconds = 1000 * 60;

        return (int)Math.abs(time / minMillSeconds);
    }
    ...
    public static java.sql.Date parse(String str, String pattern, Locale locale) {
        if(str == null || pattern == null) {
            return null;
        }
        try {
            Date date = new SimpleDateFormat(pattern, locale).parse(str);
    ...
    public static String getFromateDate(Date date,String dateFromate) {
        SimpleDateFormat formatter = new SimpleDateFormat(dateFromate);
        String strDate = formatter.format(date);
        return strDate;
    }
    /**
     ...
     * @return
     */
    public static String getConvertDateFormat(String formate, Date time) {
    ...
    public static Date getCurrentMonthFirstDayDate() {
        Calendar calendar = Calendar.getInstance();
        calendar.set(Calendar.DAY_OF_MONTH, calendar.getActualMinimum(Calendar.DAY_OF_MONTH));
        String str = getConvertDateFormat("yyyy-MM-dd", calendar.getTime());
        return convert(str);
    }
     ...
     * @return
     */
    public static Date getBeforeMonthFirstDayDate() {
        Calendar cal = Calendar.getInstance();
        Date date = new Date();
        cal.setTime(date);
        int year = 0;
    ...
    public static void main(String[] args) {
        Date payDoneTime = DateUtil.convertByFormat("2016-08-18 01:11:11",DATE_PATTERN_yyyy_MM_dd_HH_MM_ss);

        Date date = DateUtil.getSpecifiedDateByDays(DateUtil.getCurrentDateByFormat(DateUtil.DATE_PATTERN_yyyy_MM_dd), -1);

        String payDoneTimeStr = DateUtil.convert(payDoneTime, DateUtil.DATE_PATTERN_yyyy_MM_dd);
        Date payDoneTimeFormat = DateUtil.convertByFormat(payDoneTimeStr,DateUtil.DATE_PATTERN_yyyy_MM_dd);
        if (DateUtil.calculateDayCntWithoutAbs(payDoneTimeFormat,date) < 0){
    ...
    public static String  getKey(int i, String dateStr) {
        String key = null;
        if (i == 0){
            key = "今天";
        }else if (i == 1){
            ...
            key = dateStr.substring(5,dateStr.length());
        }
        return key;
    }
    ...
    public static Date getSpecifiedDateByMin(Date specifiedDay, int min) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(specifiedDay);
    ...
    public static String getSpecifiedMinByMin(Date specifiedDay, int min) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(specifiedDay);
        calendar.add(Calendar.MINUTE, min);
        return convertToHHMM(calendar.getTime());
    }

    public static String getSpecifiedDateStrByDays(Date specifiedDay, int dayCount) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(specifiedDay);
        calendar.add(Calendar.DAY_OF_YEAR, dayCount);
        return getConvertDateFormat(DATE_PATTERN_yyyy_MM_dd,calendar.getTime());
    }
     ...
     * @return
     */
    public static String getCurrentQuarterEndTime(int year,int quarter) {
        ...
        try {
            ...
            return convert(c.getTime(),DateUtil.DATE_PATTERN_yyyy_MM_dd);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "ERROR";
    ...
    public static String convert(Date date,String formatStr) {
    ...
    /**
     * 这个方法用于获得当前时间到明天零点零分零秒之间的秒数
     */
    public static int getEndOfDaySeconds() {
    ...
    /**
     * 这个方法在给定的日期基础上，增加或减少指定的年数，返回计算后得到的日期
     *
     * @param specifiedDay 要操作的起点日期
     * @param monthCount   向前或向后的月数
     * @author yinhao@HF
     * @since 2014-08-22
     */
    public static Date getSpecifiedDateByYear(Date specifiedYear, int yearCount) {
        Calendar calendar = Calendar.getInstance();
     ...
     * @return
     */
    public static int getNowToCurrentMonthLastDay(){
        Calendar calendar = Calendar.getInstance();
    ...
    public static Date getForwardDayByDayNum(Date time,int dayNum){
        String formalTime = convertToSS(time);
        //HH-mm-ss
        String hmsTime = new SimpleDateFormat("HH:mm:ss").format(time);
        //yyyy-MM-dd
        String specifiedDay = getSpecifiedDayByNum(formalTime, dayNum);
        specifiedDay = specifiedDay + " " + hmsTime;
        return DateUtil.convertByFormat(specifiedDay, DATE_PATTERN_yyyy_MM_dd_HH);
    }

    public static List<String> subTimeBy30Min(String startTime,String endTime){
        Assert.hasText(startTime);
        Assert.hasText(endTime);
        Assert.isTrue(startTime.contains(":"));
        Assert.isTrue(endTime.contains(":"));
        ...
        for (int i = startHours; i <= endHours; i++) {
            String restTimeStr1;
            String restTimeStr2;
            if ("00".equals(startArr[1])){
                restTimeStr1 = i + ":00";
                if (restTimeStr1.equals(endTime)){
                    timeList.add(endTime);
                    break;
                }

                timeList.add(restTimeStr1);

                restTimeStr2 = i + ":30";
                if (restTimeStr2.equals(endTime)){
                    timeList.add(endTime);
                    break;
                }

                timeList.add(restTimeStr2);
            }else {
                restTimeStr1 = i + ":30";
                if (restTimeStr1.equals(endTime)){
                    timeList.add(restTimeStr1);
    ...
    /**
     * 如参数是：2016-01-14 19:00-19:30
     * 返回数组为[当前年份-01-14 19:00,当前年份-01-14 19:30,]
     */
    public static List<Date> convertDateRange(String dateRange){
        Assert.hasText(dateRange);
        String dateAndMonth = dateRange.substring(0,10);
        String start = dateRange.substring(dateRange.length()-11, dateRange.length()-6);
        int length = dateRange.length();
        String end = dateRange.substring(dateRange.length()-5, length);
        Date startTime = convertByFormat(dateAndMonth+" "+start,DATE_PATTERN_yyyy_MM_dd_HH_MM);
        Date endTime = convertByFormat(dateAndMonth+" "+end,DATE_PATTERN_yyyy_MM_dd_HH_MM);
    ...
    public static Map<String,Integer> splitDate(Date date){
        Calendar now = Calendar.getInstance();
        now.setTime(date);
        Map<String,Integer> result = new HashMap<String, Integer>(4);
        result.put("YEAR",now.get(Calendar.YEAR));
        result.put("DATE",now.get(Calendar.DAY_OF_MONTH));
        int month = now.get(Calendar.MONTH) + 1;
        result.put("MONTH", month);
        result.put("QUARTER",(month-1) / 3 + 1);
        return result;
    }
     ...
     * @param map
     */
    public static void getDateMinusMonth(HashMap<String ,Integer> map,Date date){
        Calendar c = Calendar.getInstance();
        c.setTime(date);
        c.add(Calendar.MONTH, map.get("MINUSMONTH"));
        int month = c.get(Calendar.MONTH) + 1;
        map.put("MONTH",month);
        map.put("YEAR",c.get(Calendar.YEAR));
        map.put("YEAR",(month-1) / 3 + 1);
    }
     ...
     * @return
     */
    public static Date getBeforeDate(int minusMonth) {
        // 实例一个日历单例对象
        Calendar cal = Calendar.getInstance();
        Date date = new Date();
        cal.setTime(date);
        ...
        String endDay = year + "-" + month + "-" + day;
        return convertByFormat(endDay,"yyyy-MM-dd" );
    }
    ...
    public static String getFriendly(Date date){
        ...
        long second = (today.getTime().getTime()-calendar.getTime().getTime())/1000;
        if(second<60){
            return "刚刚";
        }
        long min = second/60;
